...
void
ChangeCapital(std::map<std::string, std::string> &countries,
              const std::string &country, const std::string &capital) {
  if (countries.count(country) == 0) {
    ...
    std::cout << "Country " << country << " has changed its capital from "
              << countries[country] << " to " << capital << std::endl;
  }
  countries[country] = capital;
}

void
Rename(std::map<std::string, std::string> &countries,
...
void
About(std::map<std::string, std::string> &countries,
...
void
Dump(std::map<std::string, std::string> &countries) {
  if (countries.empty()) {

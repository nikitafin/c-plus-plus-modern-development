...
ForwardIterator
max_element_if(ForwardIterator first, ForwardIterator last, UnaryPredicate pred)
{
    //  auto result = last;
    //  for (auto temp = first; temp != last; ++temp) {
    //    if (pred(*temp) and *temp > *result) {
    //      result = temp;
    //    }
    //  }
    //
    ...
    return std::max_element(
        ...
        { return (pred(rhs) and pred(lhs) and (rhs >= lhs)); });
}
...
void TestSeveralMax()
{
    ...
    const forward_list<string> text{

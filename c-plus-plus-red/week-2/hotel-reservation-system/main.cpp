...
public:
  ...
  void Book(int64_t time, const std::string &hotel_name, int64_t client_id,
            int64_t room_count);
...
void
HotelReservationSystem::Book(int64_t time, const std::string &hotel_name,
                             int64_t client_id, int64_t room_count) {
  current_time = time;
...
HotelReservationSystem::updateReservationQuery() {
  while (!reservations.empty() &&
         reservations.front().time <= current_time - kSecInDay) {

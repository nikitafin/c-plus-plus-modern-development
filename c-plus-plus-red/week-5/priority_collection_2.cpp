...
private:
  ...
  struct MyOp {
    bool operator()(Id const lhs, Id const rhs) const { return *lhs < *rhs; };
  };
...
public:
  ...
  bool IsValid(Id id) const { return m_id.find(id) != m_id.cend(); }

  const T &Get(Id id) const { return *id; }

  void Promote(Id id) {
    PriorityId old_priority_id = m_id[id];
    Priority new_priority{old_priority_id->object_id,
                          old_priority_id->priority + 1, old_priority_id->dt};

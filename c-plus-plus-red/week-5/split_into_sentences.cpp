...
std::vector<Sentence<Token>> SplitIntoSentences(std::vector<Token> tokens)
{
    ...
    for (auto it = tokens.begin(); it != tokens.end();)
    {
        ...
        auto punctuation_it = std::find_if(
            it,
            tokens.end(),
            [](auto & token) { return token.IsEndSentencePunctuation(); });
        if (punctuation_it != tokens.end())
        {
            punctuation_it = std::find_if(
                punctuation_it,
                tokens.end(),
                [](auto & token) { return !token.IsEndSentencePunctuation(); });
        }
...
void TestSplitting()
{
    ASSERT_EQUAL(
        SplitIntoSentences(
            std::vector<TestToken>({{"Split"}, {"into"}, {"sentences"}, {"!"}})),
        std::vector<Sentence<TestToken>>({{{"Split"}, {"into"}, {"sentences"}, {"!"}}}));

    ASSERT_EQUAL(
        SplitIntoSentences(
            std::vector<TestToken>({{"Split"}, {"into"}, {"sentences"}, {"!", true}})),

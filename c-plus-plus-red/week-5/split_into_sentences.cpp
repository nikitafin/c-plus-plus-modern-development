...
template <typename Token>
std::vector<Sentence<Token>>
SplitIntoSentences(std::vector<Token> tokens) {
  ...
  for (auto it = tokens.begin(); it != tokens.end();) {
    ...
    if (punctuation_it != tokens.end()) {
      punctuation_it =
          std::find_if(punctuation_it, tokens.end(), [](auto &token) {
            return !token.IsEndSentencePunctuation();
          });
    }
...
std::ostream &
operator<<(std::ostream &stream, const TestToken &token) {
...
TestSplitting() {
  ...
  ASSERT_EQUAL(
      SplitIntoSentences(std::vector<TestToken>({{"Split"},
                                                 {"into"},
                                                 {"sentences"},
                                                 {"!", true},
                                                 {"!", true},

...
int64_t
CalculateMatrixSingleThread(const Matrix &matrix) {
  int64_t result = 0;
...
CalculateMatrixSum(const vector<vector<int>> &matrix) {
  ...
  for (auto page : Paginate(matrix, page_size)) {
    futures.push_back(
        async([=]() { return CalculateMatrixSingleThread(page); }));
  }
  int64_t result = 0;
  for (auto &f : futures) {
    result += f.get();
...
void
GenerateSingleThread(ContainerOfVectors &result, size_t first_row,
                     size_t column_size) {
  for (auto &row : result) {
...
vector<vector<int>>
GenerateMultiThread(size_t size, size_t page_size) {
